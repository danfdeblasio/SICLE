...
void SisterFinder::groupConfig(string fname){
...
bool SisterFinder::sameGroup(string group1, string group2){
...
Phylogeny_Node* SisterFinder::all_match(Phylogeny_Node* n,string s){
    if (n->leaf) {
        if (n->name.substr(0,s.length()) == s || sameGroup(n->name.substr(0,n->name.find_first_of("-")),s)) {
            return n;
        } else {
            return NULL;
        }
...
SisterFinder::SisterFinder(Phylogeny* p, string s, string group_file){
