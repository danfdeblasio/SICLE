...
bool is_prefix(string a, string b, bool useRegex){
    
    if(useRegex){
        regex e (b);
        //cerr << "\"" << b << "\"\t\"" << a << "\"\t" << regex_search ( a, e , regex_constants::match_any ) << endl;
        return regex_search ( a, e , regex_constants::match_any);
    }
...
Phylogeny_Node* SiClE::all_match(Phylogeny_Node* n,string s){
    if(n==NULL || null_in_error) {return NULL;}
    //cerr << null_in_error << endl;
    //cerr << "All Match: " << n->name.substr(0,n->name.find_first_of("-")) << "\t" << s << endl;
    if (n->leaf) {
        //
        if (is_prefix(n->name,s, useRegex) || sameGroup(n->name.substr(0,n->name.find_first_of("-")),s)) {
            //cerr << "YES" << endl;
            return n;
        } else {
            return NULL;
        ...
        //cerr << "All Match (DO): " << ((l==NULL)?"LN":"") << "\t" << ((r==NULL)?"RN":"") << endl;
        if(null_in_error) return NULL;
        if (l == NULL && !null_in_error) {
            return r;
        }else if(r == NULL && !null_in_error){
            return l;
        }else{
            if(l==n->left_child && r==n->right_child){
                //cerr << "Return N" << endl;
                return n;
            }
            else{
                ...
                //
                //cout << n->print() << endl;
                if(r==n->right_child && !rerooted){
                    //cout << "REDUX -L " << endl;
                    if(l->parent->left_child == l){
                        phylo->root = Phylogeny::reroot(l->parent,l->parent->right_child,phylo->root);
                        
                    }else{
                        phylo->root = Phylogeny::reroot(l->parent,l->parent->left_child,phylo->root);
                    }
                    rerooted = true;
                    
                    //cout << "HERE?" << endl;
                    return all_match(phylo->root,s);
                }
                else if(l==n->left_child && !rerooted){
                    
                    //cout << "REDUX -R " << endl;
                    if(r->parent->left_child == r){
                        phylo->root = Phylogeny::reroot(r->parent,r->parent->right_child,phylo->root);
                    }else{
                        phylo->root = Phylogeny::reroot(r->parent,r->parent->left_child,phylo->root);
                    }
                    rerooted = true;
...
void SiClE::get_sisters(){
    ...
    string extra_sister = "";
    if (target != NULL) {
        //cout << ((target->parent == phylo->root)?"1":"2") << endl;
        //cout << phylo->root->print() << endl;
        ...
        if (target->parent == phylo->root) {
            ...
            if (1==1 || target->parent->parent == phylo->root) {
                ...
                //cout << ">>" << ((to_print->parent != NULL)?"1":"0") <<endl;
                if(to_print != NULL && to_print->parent != NULL && to_print->parent->parent != NULL){
                    //cout << "MORE?" << endl;
                    ...
                    while(to_print->parent->parent != phylo->root && extra_sister != ""){
                        if(count > 10 || to_print->parent->parent == NULL){
                            //cerr << phylo->root->print() << endl;
                            break;
                        }
                        count++;
                ...
                }else if(target->parent->parent == phylo->root){
                    extra_sister = to_print->name;
                }
...
SiClE::SiClE(Phylogeny* p, string s, string group_file, bool uR){
    useRegex = uR;
    groupConfig(group_file);
    phylo = p;
    
    //cout << p->root->print() << endl <<  endl;
    
    target = all_match(p->root,s);
    //cerr << "HERE??" << endl;
    if(rerooted){ null_in_error = false; target = all_match(p->root,s); }
    
    //cout << p->root->print() << endl;
    //cerr << "Target: " << target->print() << endl;
    //cerr << "Root: " << p->root->print() << endl;
    sister_names_count = 0;
    if(target != p->root && target != NULL){
        get_sisters();
    }else if(target != p->root){
        cerr << "CLADE NOT FOUND" << endl;
    }else{
        cerr << "TARGET IS ROOT" << endl;
    }
}
...
SiClE::SiClE(Phylogeny* p, string s, bool uR){
    ...
    target = all_match(p->root,s);
    if(rerooted){ null_in_error = false; target = all_match(p->root,s); }
    //cout << "BEFORE" << target->print() << endl << endl;
    sister_names_count = 0;
    if(target != p->root && target != NULL){
        //cerr << "Target: " << target->print() << endl;
        //cerr << "Root: " << p->root->print() << endl;
